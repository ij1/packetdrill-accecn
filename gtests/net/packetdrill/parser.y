...
int accecn_counter_order[MAX_TCP_ACCECN_COUNTERS] = {
						...
						char **error, bool exp)
{
	...
	for (i = 0; i < sizeof(accecn_counter_order) / sizeof(int); i++) {
		if (cnts->present & (1 << accecn_counter_order[i]))
			counters = i + 1;
		else
			cnts->counter[accecn_counter_order[i]] = 0;
	}
	option_bytes = counters * sizeof(struct accecn_counter) +
				TCPOLEN_EXP_ACCECN_BASE;
	option = tcp_option_new(TCPOPT_EXP, option_bytes);
	if (exp)
		option->data.accecn_exp.magic = htons(TCPOPT_ACCECN_MAGIC);

	for (i = 0; i < counters; i++) {
		u32 val = cnts->counter[accecn_counter_order[i]];
...
%token <reserved> URG MD5 FAST_OPEN FAST_OPEN_EXP ACCECN_EXP
...
%type <tcp_accecn_counters> accecn_counter_list accecn_counter
...
| ACCECN_EXP accecn_counter_list {
	char *error = NULL;
	$$ = new_tcp_accecn_option(&$2, &error, true);
